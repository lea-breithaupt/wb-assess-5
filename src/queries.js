...
export const query8 = await Human.findAll({where: {[Op.not]: {email: {[Op.like]: '%gmail%'}}}});
console.log(query8)
...
export async function printHumansAndAnimals() {
    ...
    for(const human of humans){
        const ani = human.animal;
        if(ani) {
            console.log(human.fname, human.lname, ani.name, ani.species);
        }
    }
}
